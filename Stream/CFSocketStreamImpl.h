...
typedef enum { kSSLIdle = 1, kSSLHandshake, kSSLConnected, kSSLClosed, kSSLAborted } SSLSessionState;
...
#define __socketGetFD(ctxt) \
  ((int)((__CFBitIsSet(ctxt->flags, CREATED_WITH_SOCKET)) ? ctxt->u.sock : (ctxt->sock ? CFSocketGetNative(ctxt->sock) : -1)))
